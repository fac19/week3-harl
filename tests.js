...
test("Submitting a new task adds the correct template to the list", t => {
  addTask("low", "Clean the fridge", false);
  let thePriority =
    document.querySelector(".to-do-list > li:last-child > div") !== null;
  let theText =
    document.querySelector(".to-do-list > li:last-child > p") !== null;
  let theStatus =
    document.querySelector(".to-do-list > li:last-child > input") !== null;
...
test("Checking an entry marks it as complete", t => {
  // test goes here
  addTask("low", "Clean the fridge", false);
  const checkBoxTest = document.querySelector(".to-do-list__card__checkbox")
//   checkBoxTest.click()
});
